...
public class ShopMainListActivity extends NoHttpFragmentBaseActivity implements IndicatorViewPager.OnIndicatorPageChangeListener {
    ...
    private void initView() {
        ...
        ctx = this;
        UIControlUtils.UITextControlsUtils.setUIText(findViewById(R.id.title), ActivityConstans.UITag.TEXT_VIEW, "糖心车轮");
        findViewById(R.id.topPanel).setVisibility(View.VISIBLE);
        findViewById(R.id.shopCartBtn).setVisibility(View.VISIBLE);
    ...
    private void initMenu() {
        ...
        menuList.add(new PhoneCommBaseType(1, "热销"));
        menuList.add(new PhoneCommBaseType(2, "新品"));
    }
    ...
    private class MyAdapter extends IndicatorViewPager.IndicatorFragmentPagerAdapter {
        ...
        public Fragment getFragmentForPage(int position) {
            ...
            if (fragmentList.get(menuList.get(position).getTypeName()) != null) {
                mainFragment = (ShopMainFragment) fragmentList.get(menuList.get(position).getTypeName());
            } else {
    ...
    @OnClick({R.id.back,R.id.customerServiceBtn,R.id.collectBtn,R.id.shopCartBtn})
    public void OnClick(View v) {
        switch (v.getId()) {
            case R.id.back:
                onBackPressed();
                break;
            case R.id.shopCartBtn:
