 ...
 * 礼品汇兑换记录列表适配器
 */
public class WashHasCommentListAdapter extends BaseAdapter {
    private List<PhoneGoodEntity> list;
    private Context ctx;

    public WashHasCommentListAdapter(Context ctx, List<PhoneGoodEntity> list) {
        this.list = list;
        this.ctx = ctx;
    }
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            vh = new ViewHolder();
            convertView = LayoutInflater.from(ctx).inflate(R.layout.wash_has_comment_item, null);
